    ...
    <CContainer fluid className="px-4 py-4">
      ...
      {activeTab === 'users' && isAdmin && (
        <CCard className="shadow-sm border-0">
           ...
           <CCardBody className="p-0"><div className="admin-table-container border-0 mb-0"><table className="admin-table"><thead><tr><th className="ps-4">Name</th><th>Username</th><th>Role</th><th className="text-center">Status</th><th className="text-end pe-4">Action</th></tr></thead><tbody>{loadingUsers ? (<tr><td colSpan="5" className="text-center py-5"><CSpinner color="primary"/></td></tr>) : filteredUsers.length === 0 ? (<tr><td colSpan="5" className="text-center py-5 text-muted">No users found.</td></tr>) : (filteredUsers.map(u => (<tr key={u.id}><td className="ps-4"><div className="d-flex align-items-center"><div className="bg-light rounded-circle p-1 me-2 border d-flex justify-content-center align-items-center overflow-hidden" style={{width: '32px', height: '32px'}}>{u.avatar ? <img src={u.avatar.startsWith('http') ? u.avatar : resolveAssetUrl(u.avatar)} alt="" style={{width: '100%', height:'100%', objectFit: 'cover'}}/> : <CIcon icon={cilUser} size="sm" className="text-secondary"/>}</div><div className="fw-bold text-dark">{u.first_name} {u.last_name}</div></div></td><td className="text-muted font-monospace small">{u.username}</td><td><CBadge color={u.role === 'admin' ? 'danger' : u.role === 'driver' ? 'warning' : 'info'} shape="rounded-pill" className="text-uppercase px-3">{u.role}</CBadge></td><td className="text-center"><span className={`status-badge ${u.status === 'Active' ? 'active' : 'cancelled'}`}>{u.status}</span></td><td className="text-end pe-4"><button className="btn-brand btn-brand-outline btn-brand-sm" onClick={() => openEditUser(u)}><CIcon icon={cilPencil}/></button></td></tr>)))}</tbody></table></div></CCardBody>
        </CCard>
      )}
